...
import { getAllOrders, getMyTodayOrderCount } from "../../services/OrderService";
import { openShift, closeShift, getCurrentShift } from "../../services/ShiftService";
...
const SalesOverview = () => {
  ...
  const [orders7Days, setOrders7Days] = useState({ series: [], categories: [] });
  const [ordersByHourToday, setOrdersByHourToday] = useState(Array(24).fill(0));
  const [paymentMethodToday, setPaymentMethodToday] = useState({ CASH: 0, MOMO: 0, OTHER: 0 });
  ...
  const fetchOrdersToday = async (accountIdLocal = null) => {
    try {
      ...
      const res = await getAllOrders({ from: formatDate(start), to: formatDate(end) });
      const data = res.result || res || [];
      const myId = accountIdLocal || user?.id || null;
      const filtered = data.filter(o => {
        const uid = o.accountId || (o.account && o.account.accountId) || null;
        const paid = (o.paymentStatus || '').toString().toLowerCase() === 'đã thanh toán';
      ...
      filtered.forEach(o => {
        const dt = new Date(o.orderDate || o.createdDate || o.createdAt || Date.now());
        const h = dt.getHours();
        byHour[h] += 1;
      });
      ...
      filtered.forEach(o => {
        const method = (o.payment?.paymentMethod || o.paymentMethod || "OTHER").toString().toUpperCase();
        if (method.includes("MOMO")) pm.MOMO += 1; else if (method.includes("CASH") || method.includes("TIỀN MẶT")) pm.CASH += 1; else pm.OTHER += 1;
      });
      setPaymentMethodToday(pm);
    } catch (e) {
  ...
  const totalRevenueToday = ordersToday.reduce((sum, o) => sum + (o.totalAmount || 0), 0);

  const handleOpenShift = async () => {
    if (!shiftAmount || Number(shiftAmount) < 0) { message.error("Nhập số tiền mặt hợp lệ"); return; }
    try {
  ...
  const handleCloseShift = async () => {
    if (shiftAmount === undefined || Number(shiftAmount) < 0) { message.error("Nhập số tiền mặt hiện tại hợp lệ"); return; }
    try {
      ...
      <div className="content">
        <div className="page-header">
          <div className="add-item d-flex justify-content-between align-items-center">
            <div className="page-title">
              <div className="mb-3">
                <h1 className="mb-1">Chào mừng, {user?.fullName || 'Nhân viên'}</h1>
                <p className="fw-medium">
                  Bạn đã bán <span className="text-primary fw-bold">{myTodayCount}+</span>{" "}
                  đơn hàng hôm nay
                </p>
              </div>
            </div>
        ...
        <div className="row">
          <div className="col-xl-6 col-lg-6">
            {!user ? (
              <div className="text-center py-4"><Spin size="large" /></div>
            ) : (
                  ...
                  <div className="card bg-primary text-white">
                    <div className="card-body">
                      <div className="d-flex justify-content-between align-items-center">
                        <div>
                          <h6 className="text-white mb-2">Tổng doanh thu hôm nay</h6>
                          <h3 className="text-white mb-0">{loadingOrders ? <Spin /> : formatCurrency(todayRevenue)}</h3>
                        </div>
                ...
                <div className="col-md-6">
                  <div className="card bg-success text-white">
                    <div className="card-body">
                      <div className="d-flex justify-content-between align-items-center">
                        <div>
                          <h6 className="text-white mb-2">Số lượng đơn hôm nay</h6>
                          <h3 className="text-white mb-0">{loadingOrders ? <Spin /> : myTodayCount}</h3>
                        </div>
                        <div className="avatar-lg" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div className="card">
              <div className="card-header d-flex align-items-center justify-content-between flex-wrap gap-3">
                <div className="d-inline-flex align-items-center">
                  <span className="title-icon bg-soft-teal fs-16 me-2"><i className="ti ti-clock" /></span>
              ...
              <div className="card-body">
                {shiftLoading ? (
                  ...
                  <div className="row g-3">
                    ...
                    <div className="col-12">Bắt đầu: {shift.openedAt ? new Date(shift.openedAt).toLocaleString('vi-VN') : ''}</div>
                    <div className="col-12">Tiền mặt ban đầu: {formatCurrency(shift.initialCash)}</div>
                  </div>
                ) : (
                  <div><span className="badge badge-secondary">Đang đóng</span></div>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="row mt-3">
          <div className="col-xl-6 col-lg-6">
            <div className="card">
              <div className="card-header d-flex align-items-center justify-content-between">
                <div className="d-inline-flex align-items-center">
                  <span className="title-icon bg-soft-info fs-16 me-2"><i className="ti ti-chart-bar" /></span>
          ...
          <div className="col-xl-6 col-lg-6">
            <div className="card">
              <div className="card-header d-flex align-items-center justify-content-between">
                <div className="d-inline-flex align-items-center">
                  <span className="title-icon bg-soft-teal fs-16 me-2"><i className="ti ti-calendar-stats" /></span>
              ...
              <div className="card-body">
                <Chart
                  ...
                  series={[{ name: 'Đơn hàng', data: orders7Days.series || [] }]}
                  type="bar"
                  height={240}
                />
              </div>
        ...
        >
          {shiftLoading ? (
            ...
            <div>
              ...
              <div className="mb-2">Bắt đầu: {shift.openedAt ? new Date(shift.openedAt).toLocaleString('vi-VN') : ''}</div>
              <div className="mb-3">Tiền mặt ban đầu: {formatCurrency(shift.initialCash)}</div>
              <label className="form-label fw-bold">Tiền mặt hiện tại</label>
              <input type="number" className="form-control" value={shiftAmount} onChange={e => setShiftAmount(e.target.value)} />
              <div className="text-end mt-3">
                <button className="btn btn-purple" onClick={handleCloseShift}>Đóng ca</button>
              </div>
            </div>
          ) : (
            <div>
              ...
              <input type="number" className="form-control" value={shiftAmount} onChange={e => setShiftAmount(e.target.value)} />
              <div className="text-end mt-3">
                <button className="btn btn-teal" onClick={handleOpenShift}>Mở ca</button>
              </div>
            </div>
          )}
        </Modal>
