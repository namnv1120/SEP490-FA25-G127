...
public class OrderController {
    ...
    @GetMapping("/returns")
    @PreAuthorize("hasAnyRole('Quản trị viên','Chủ cửa hàng','Nhân viên bán hàng')")
    public ApiResponse<List<OrderResponse>> getReturnOrders(
            @RequestParam(required = false) String searchTerm,
            @RequestParam(required = false) String orderStatus,
            @RequestParam(required = false) String from,
            @RequestParam(required = false) String to) {
        ...
        // If any search parameter is provided, use search method
        if (searchTerm != null || orderStatus != null || fromDate != null || toDate != null) {
            response.setResult(orderService.searchReturnOrders(searchTerm, orderStatus, fromDate, toDate));
            response.setMessage("Tìm kiếm đơn hoàn hàng thành công.");
        } else {
            response.setResult(orderService.getAllOrders());
            response.setMessage("Lấy danh sách đơn hoàn hàng thành công.");
        }
    ...
    @PostMapping("/{id}/mark-for-return")
    @PreAuthorize("hasAnyRole('Quản trị viên','Chủ cửa hàng','Nhân viên bán hàng')")
    public ApiResponse<OrderResponse> markForReturn(@PathVariable UUID id) {
        ApiResponse<OrderResponse> response = new ApiResponse<>();
        response.setResult(orderService.markForReturn(id));
        response.setMessage("Đã đánh dấu đơn hàng chờ hoàn.");
        return response;
    }

    @PostMapping("/{id}/revert-return")
    @PreAuthorize("hasAnyRole('Quản trị viên','Chủ cửa hàng','Nhân viên bán hàng')")
    public ApiResponse<OrderResponse> revertReturnStatus(@PathVariable UUID id) {
        ApiResponse<OrderResponse> response = new ApiResponse<>();
        response.setResult(orderService.revertReturnStatus(id));
        response.setMessage("Đã xóa phiếu hoàn hàng.");
        return response;
    }
